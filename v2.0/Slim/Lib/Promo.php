...
class Promo extends \Slim\Helper\Lib
{
    ...
    */
    ...
    /**
     * @method Отдельный запрос на регистрацию по промокоду уже существующему юзеру
     * @return array
     */
    public static function registerPromo()
    {
        global $USER;
        ...
        //Если промокод уже был зарегистрирован, вернём ошибку
        if ($arUser['UF_PROMO_INVITE']) {
            \Slim\Helper\ErrorHandler::put('PROMO_USER_ALREADY_REGISTER', 400);
            return array();
        }
        
        //Если прошло не более 24 часов с момента регистрации
        if (strtotime($arUser['DATE_REGISTER']) + 24 * 60 * 60 >= time()) {
            $promoID = self::getIdByCode($userCode);
            
            if ($promoID) {
                $user = new \CUser;
                $user->Update($USER->GetID(), array('UF_PROMO_INVITE' => $promoID));
                
                self::sendRegisterTo1C($userCode);
                
                return array();
            }
        } else {
            \Slim\Helper\ErrorHandler::put('PROMO_USER_REGISTER_EXPIRED', 400);
            return array();
        }
        
        \Slim\Helper\ErrorHandler::put('PROMO_USER_NOT_REGISTER', 400);
        return array();
    }
