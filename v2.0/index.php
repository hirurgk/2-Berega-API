...
$app->group('/order', function() use ($app) {
    ...
    /* Оценка заказа */
    $app->post('/rate', 'auth', function() {
        $arResult = \Slim\Lib\Order::rate();
        \Slim\Helper\Answer::json( $arResult, false, 'order' );
    });
    
    /* Повтор заказа */
    $app->get('/repeat/:id', 'auth', function( $id ) {
        $arResult = \Slim\Lib\Order::repeat( $id );
        \Slim\Helper\Answer::json( $arResult, false, 'order' );
    });
    
    /* Статус оплаты заказа */
    $app->get('/payment/:id', function( $id ) {
        $arResult = \Slim\Lib\Order::getPaymentStatus( $id );
        \Slim\Helper\Answer::json( $arResult, false, 'order' );
    });
    
    /* Смена статуса заказа ДЛЯ 1С */
    $app->get('/status', function() {
        \Slim\Lib\Order::changeStatusFrom1C();
    });
});
...
$app->group('/promo', function() use ($app) {
    ...
    /* Регистрация промокода существующему юзеру */
    $app->post('/register', 'auth', function() {
        $promo = \Slim\Lib\Promo::registerPromo();
        \Slim\Helper\Answer::json( $promo, false, 'promo' );
    });
});
...
/* группа для работы с контролем качества */
$app->group('/feedback', function() use ($app) {
    /* Принимает сообщение от клиента */
    $app->post('', function() {
        $sms = \Slim\Lib\Feedback::take();
        \Slim\Helper\Answer::json( $sms, false, 'feedback' );
    });
});
