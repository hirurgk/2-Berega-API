...
class Catalog extends \Slim\Helper\Lib
{
     ...
     *
     */
    public static function getGoodsList( $catId = false, $goodsId = false )
    {
        global $DB;
        ...
        //Запрашиваемые поля
        $arSelect = array(
            'ID','NAME', 'SORT', 'TIMESTAMP_X', 'ACTIVE', 'PROPERTY_NAME_MOBILE', 'PROPERTY_MOBILE_PREVIEW', 'PROPERTY_MOBILE_DETAIL',
        ...
        foreach( $product_info as $res )
        {
            ...
            $item = self::setItem( $res, $catalogGroup );
    ...
    public static function getRecommendationIds( $count = 20)
    {
		...
		$newProdusctIds = array();
		foreach ($productIds as $productId) {
			if ($productId > 900000) {
				$newProdusctIds[] = (int)$productId - 900000;
			} else {
				$newProdusctIds[] = (int)$productId;
			}
		}

        if( count($newProdusctIds) > 100 ) {
            \Slim\Helper\ErrorHandler::put('INCORRECT_REQUEST');
            return;
        }

        # Получим ID рекомендаций из API
        $obj = new \Slim\Helper\Retailrocket( $siteId, $newProdusctIds );
     ...
     * @return array
     */
    private static function setItem( $res,  $catalogGroup )
    {
        $item = array();
